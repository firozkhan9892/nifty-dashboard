...
STOCK_KEYWORDS = {
    ...
    "ORDERWIN": ["orderwin", "order win"], "BADA CONTRACT": ["bada contract"],
    "GOVT DEAL": ["govt deal", "government deal", "govt. deal"],
    "BSE SENSEX": ["sensex", "bse sensex"], "NIFTY 50": ["nifty 50", "^nsei"],
}

FOCUS_STOCKS = ["ORDERWIN", "BADA CONTRACT", "GOVT DEAL"]
...
def get_deal_signal(order_value_cr):
    if order_value_cr is None:
        return "📋 NOT DISCLOSED", "🟡 NEUTRAL"
    if order_value_cr >= MARKET_CAP_RULES["BUY"]:
        return f"💎 ₹{order_value_cr:,.0f} CR", "🟢 BUY ZONE"
    elif order_value_cr >= MARKET_CAP_RULES["WATCH"]:
        return f"📊 ₹{order_value_cr:,.0f} CR", "🟡 WATCH"
    else:
        return f"🔸 ₹{order_value_cr:,.0f} CR", "⚪ IGNORE"

def is_focus_stock(stocks_list):
    if not stocks_list:
        return False
    return any(f in stocks_list for f in FOCUS_STOCKS)

def should_ignore_news(order_value_cr):
    if order_value_cr is None:
        return False
    return order_value_cr < MARKET_CAP_RULES["WATCH"]
...
def api_orders():
    ...
    for a in get_cached_news():
        ...
        if any(kw in combined for kw in ORDER_KEYWORDS):
            order_value = extract_order_value(combined)
            stocks = detect_affected_stocks(combined)
            sentiment = analyze_news_sentiment(title, desc)
            
            if should_ignore_news(order_value):
                continue
            
            value_str, signal = get_deal_signal(order_value)
            is_focus = is_focus_stock(stocks)
            ...
            order_data.append({
                ...
                "signal": signal,
                "is_focus": is_focus,
                "priority": "HIGH" if order_value and order_value >= 500 else "MEDIUM" if order_value and order_value >= 100 else "LOW"
            })
    
    order_data.sort(key=lambda x: x.get("raw_value") or 0, reverse=True)
    return jsonify(order_data[:10])

@app.route("/api/india-signals")
def api_india_signals():
    focus_data = []
    for a in get_cached_news():
        title = a.get("title", "")
        desc = a.get("description", "") or ""
        ...
        if not any(kw in combined for kw in ORDER_KEYWORDS):
            continue
    ...
    focus_data.sort(key=lambda x: x.get("raw_value") or 0, reverse=True)
    return jsonify(focus_data[:15])
...
def auto_telegram():
    while True:
        try:
            ...
            if focus_data:
                msg = "🇮🇳 *INDIA DEAL ALERT*\n━━━━━━━━━━━━━━━━━━\n"
                for item in focus_data[:3]:
                    stock_list = ", ".join(item["stocks"][:2]) if item["stocks"] else "General"
                    msg += f"{item['signal']} | {item['value']}\n"
                    msg += f"🏷️ {stock_list}\n\n"
                msg += "━━━━━━━━━━━━━━━━━━"
...
async def cmd_india(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not focus_data:
            await update.message.reply_text("📭 No significant orders/deals found (500cr+)\n\nFocus: ORDERWIN, BADA CONTRACT, GOVT DEAL", parse_mode="Markdown")
            return
        
        msg = "🇮🇳 *INDIA STOCK SIGNALS*\n━━━━━━━━━━━━━━━━━━\n"
        msg += "📊 500cr+ = BUY | 100-500cr = WATCH\n\n"
        
        for item in focus_data[:8]:
            stock_list = ", ".join(item["stocks"][:2]) if item["stocks"] else "General"
            msg += f"━━━━━━━━━━━━━━━━━━\n"
            msg += f"{item['signal']}\n"
            msg += f"💰 {item['value']}\n"
        ...
        logger.error(f"India command error: {e}")
...
def run_reply_bot():
    ...
    app_bot.add_handler(CommandHandler("india", cmd_india))
